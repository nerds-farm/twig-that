...
class Block extends Module_Base {
    ...
    public function twig_settings() {
        require_once(__DIR__ . DIRECTORY_SEPARATOR . 'page' . DIRECTORY_SEPARATOR . 'settings.php');
    }
    ...
    public function meta_fields_save_meta_box_data($post_id, $post, $update) {
        ...
        if (!empty($_POST['_block_supports'])) {
            $tmp = array_keys($_POST['_block_supports']);
            foreach ($tmp as $sup) {
                switch ($sup) {
                    case 'dimensions':
                    default:
        ...
        foreach ($assets as $asset => $type) {
            ...
            if ($asset == 'editorScript') {
                if (strpos($code, 'generated by '.$plugin_name) !== false) {
